...
# set rule to 'disabled'

element_disabled = r'<disabled>yes</disabled>'
...
for rule in rule_list:
    ...
    response_set_disabled = pa_utils.set_config(dev_ip,api_key,xpath_rule,element_disabled)

    if response_set_disabled.get('response').get('@status')=='success':
        print (f"Rule '{rule}' is disabled. ", end='')
    else:
        print(f"Error disabling rule '{rule}'.")
    ...
    if description_result:= response_desc.get('response').get('result'):
        ...
        new_description = rule_description + '\nZero hit rule disabled'   

    else:
        new_description = 'Zero hit rule disabled'
